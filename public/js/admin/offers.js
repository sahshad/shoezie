...
document.getElementById("editOfferForm").addEventListener("submit", function (event) {
  ...
  if (offerData.value > offerData.minProductPrice) {
    Swal.fire("Error!", "value must be greater than minimum product price", "error");
    return;
  }
...
document.getElementById("editOfferType").addEventListener("change", function () {
  ...
  editMinProductPriceField.style.display =
    document.getElementById("editOfferFor").value === "Category" && this.value === "flat" ? "block" : "none";
  toggleEditOfferButton();
});
...
function openEditModal(offerId, offerFor, targetName, offerType, value, expiresAt, minProductPrice) {
  ...
  document.getElementById("editExpiresAt").value = new Date(expiresAt).toISOString().split("T")[0];
