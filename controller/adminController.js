...
async function changeUserStatus(req, res) {
  ...
  try {
    ...
    if (user.status === false) {
      if (req.session.user && req.session.user === id.toString()) delete req.session.user;
    }

    if (result)
      res
        .status(StatusCodes.OK)
        .json({ success: true, message: `User ${action === "list" ? "listed" : "unlisted"} successfully.` });
  } catch (error) {
    console.error(error);
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: "Error toggling product status.", error });
  }
}
