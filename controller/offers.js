...
async function getOffers(req, res) {
  try {
    ...
    const offers = await Offer.find().populate("targetId").skip(skip).limit(limit);
...
async function createOffer(req, res) {
  try {
    ...
    if (offerType === "percentage" && (value < 0 || value > 80)) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ success: false, message: "Percentage value must be between 0 and 80." });
    }

    if (offerType === "percentage" && maxDiscount != null && maxDiscount <= 0) {
      return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: "Max discount must be positive." });
    }
    ...
    if (offerFor === "Product") {
      ...
      if (!target) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({ success: false, message: "product not found. please check product name" });
      }

      if (value > target.price) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({ success: false, message: "Offer value is greater than product price" });
      }
      ...
      return res
        ...
        .json({ success: true, message: "Product offer applied successfully.", offer: newOffer });
      ...
      if (!target) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({ success: false, message: "category not found. please check category name" });
      }
      ...
      return res
        .status(StatusCodes.CREATED)
        .json({ success: true, message: "Category offer applied successfully.", offer: newOffer });
    }

    if (!target) {
      return res.status(StatusCodes.NOT_FOUND).json({ success: false, message: `${offerFor} not found.` });
    }
  } catch (error) {
    console.error("Error creating offer:", error);
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ success: false, message: "An error occurred while creating the offer." });
  }
}
...
async function changeOfferStatus(req, res) {
  ...
  try {
    ...
    if (!offer) {
      return res.status(StatusCodes.NOT_FOUND).json({ success: false, message: "Offer not found" });
    }
...
async function editOffer(req, res) {
  try {
    ...
    return res
      ...
      .json({ success: true, message: "Offer updated successfully.", offer: existingOffer });
  } catch (error) {
    console.error("Error editing offer:", error);
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ success: false, message: "An error occurred while editing the offer." });
  }
}
