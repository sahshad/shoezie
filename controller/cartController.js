...
async function addProductToCart(req, res) {
  ...
  try {
    ...
    if (!user) {
      return res.status(StatusCodes.NOT_FOUND).json({ success: false, message: "You are not loged in. Please Login" });
    }
    if (!product) {
    ...
    if (!size) {
      return res.status(StatusCodes.NOT_FOUND).json({ success: false, message: "Please select a size" });
    }
    ...
    if (productInCart) {
      if (size.stock <= productInCart.quantity) {
        return res
          ...
          .json({ success: false, message: `You already have ${size.stock} in your cart` });
      }
      productInCart.quantity += quantity;
    } else {
      if (size.stock < quantity) {
        return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: `Only ${size.stock} items left` });
      }
    ...
    return res.status(StatusCodes.OK).json({ success: true, message: "Product added to cart successfully" });
  } catch (error) {
...
async function removeProductFromCart(req, res) {
  ...
  try {
    ...
    if (!cart) {
      return res.status(StatusCodes.NOT_FOUND).json({ succes: false, message: "User or cart not found" });
    }
    ...
    if (result) {
      return res.status(StatusCodes.OK).json({ success: true, message: "Product removed from cart" });
    } else {
...
async function updateProductQuantity(req, res) {
  ...
  try {
    const cart = await Cart.findOne({ user: userId });
    if (!cart) {
      return res.status(StatusCodes.NOT_FOUND).json({ success: false, message: "User or cart not found" });
    }
    ...
    if (!productSize) {
      return res.status(StatusCodes.NOT_FOUND).json({ success: false, message: "Product size not found" });
    }

    if (quantity > productSize.stock) {
      return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: "Product stock limit reached" });
    }

    cartItem.quantity = quantity;
    await cart.save();
    return res.status(StatusCodes.OK).json({ success: true, message: " quantity updated successfully", cart: cart });
  } catch (error) {
